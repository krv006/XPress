...
BASE_URL = "https://back.usstartruckingllc.com/api/shipping/vehicles/years/"
OUT_FILE = "vehicle_years.json"
HEADERS = {"User-Agent": "kamron-fetch-script/1.0"}


def fetch_all_vehicle_years():
    all_results = []
    page = 1
...
def main():
    vehicle_years = fetch_all_vehicle_years()
    final_json = {"vehicle_years": vehicle_years}
    with open(OUT_FILE, "w", encoding="utf-8") as f:
        json.dump(final_json, f, ensure_ascii=False, indent=2)
    print(f"[DONE] Saved {len(vehicle_years)} vehicle years to {OUT_FILE}")
